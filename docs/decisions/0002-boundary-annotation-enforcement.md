...
- **Related:** [0001](0001-record-architecture-decisions.md); `agent-instructions/component-explainability.md`; `agent-instructions/security-practices.md`
...
5. **A matching rule in `agent-instructions/`** so Claude knows the convention and writes annotated code by default.
...
- **Easier.** New boundaries get the convention by default (Claude reads `agent-instructions/`, human reviewers trust the CI gate). Exemptions are visible. Conventions can be added incrementally without rewriting the tree.
...
- `agent-instructions/component-explainability.md` — adjacent pattern for the README-at-every-boundary rule.
- `agent-instructions/security-practices.md` — uses this pattern implicitly for security gates (e.g., the `/security-review` skill's CI coverage audit).
